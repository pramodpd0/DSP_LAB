...
(using collision technique : Double hashing)
1.Insert a Key
2. Delete a Key
...
/* Using double hashing collision resolution technique 
 'dh' returns slot number for storing key value  */
int DH(int key, int i)
{
    int dh;
    int hf1_key, hf2_key ;
    hf1_key = key%m;
    hf2_key = 1 + (key%(m-2));
    dh= (hf1_key + i*hf2_key)%m;
    return dh;
